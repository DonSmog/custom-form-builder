        ...
        {savedForms.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {savedForms.map((savedForm) => (
              <Card key={savedForm.id} className="hover:shadow-lg transition-shadow duration-200">
                <CardHeader className="pb-3">
                  ...
                  {savedForm.description && (
                    <p className="text-sm text-gray-600 line-clamp-2">{savedForm.description}</p>
                  )}
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm text-gray-500">
                      <div>
                        <span className="font-medium">Sections:</span> {savedForm.sections.length}
                      </div>
                      <div>
                        <span className="font-medium">Elements:</span>{" "}
                        {savedForm.sections.reduce(
                          (total, section) =>
                            total + section.groups.reduce((groupTotal, group) => groupTotal + group.elements.length, 0),
                          0,
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-2 border-t">
                      <Button size="sm" onClick={() => handleLoadForm(savedForm)} className="flex-1 mr-2">
                        Load Form
                      </Button>
